...
*/
	...
	drawRow1$:
		x .req r2
		mov x,#1024
		mov colour,#1111100000000000
		drawPixel$:
			strh colour,[fbAddr]
			add fbAddr,#2
			sub x,#1
			cmp x,#666
			moveq colour,#0000011111100000
			strheq colour,[fbAddr]
			addeq fbAddr,#2
			cmp x,#342
			moveq colour,#0000000000011111
			strheq colour,[fbAddr]
			addeq fbAddr,#2
			teq x,#0
			bne drawPixel$
	drawRow2$:
		x .req r2
		mov x,#1024
		mov colour,#0000000000011111
		drawPixel$:
			strh colour,[fbAddr]
			add fbAddr,#2
			sub x,#1
			cmp x,#666
			moveq colour,#1111100000000000
			strheq colour,[fbAddr]
			addeq fbAddr,#2
			cmp x,#342
			moveq colour,#0000011111100000
